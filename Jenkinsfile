pipeline {
    ...
    stage('Checkout') {
            steps {
                git branch: 'master', credentialsId: 'github-creds', url: 'https://github.com/SwapnilDA-20/backend_hadiya-products.git'
            }
        }
            ...
            steps {
                withCredentials([string(credentialsId: 'sonar-creds', variable: 'SONAR_AUTH_TOKEN')]) {
                    sh '''
                        ...
                        -Dsonar.projectName=HProduct \
                        -Dsonar.projectKey=HPBackend
                    '''
                }
            }
        }
    stage('Build and Push Docker Image') {
      environment {
        DOCKER_IMAGE = "swapnil2026/hbackend:${BUILD_NUMBER}"
    ...
    stage('Update Deployment File') {
     environment {
            GIT_REPO_NAME = "backend_hadiya-products"
